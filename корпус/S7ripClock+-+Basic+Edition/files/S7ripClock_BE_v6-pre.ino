// S7ripClock - Basic Edition - Software update v6-preview / nodeMCU support (experimental)
// 06/2020 Daniel Cikic
// Serial is set to use 74880 baud in hope to see all startup messages from nodeMCU
...
#endif
#define LED_PWR_LIMIT 650                         // 650mA - Power limit in mA (voltage is set in setup() to 5v)
...
byte upperLimitLDR = 130;                         // everything above this value will cause max brightness (according to current level) to be used (if it's higher than this)
...
unsigned long lastLoop = 0;                       // last time when main loop() has cycled through
unsigned long lastColorChange = 0;                // time when last color change was done, used for timing inside main loop()
...
void setup() {
  ...
  if ( dbg ) {
    ...
    Serial.println(F("S7tripClock starting up..."));
...
void setupClock() {
  // finally not using a custom displayTime routine for setup, improvising a bit and using the setupColor-Palette defined above
  ...
  setupTime.Day = 31;                                                                       // not really neccessary as day/month aren't used but who cares ^^
